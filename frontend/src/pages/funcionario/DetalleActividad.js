...
const DetalleActividad = () => {
    ...
    const [registrosPorPagina] = useState(10); // 10 registros por página
        ...
        const pageTitle = 'Detalle de la actividad';
        const pageDescription = <Box display="flex" alignItems="center" mb={3}>
            <IconButton onClick={() => navigate(-1)} sx={{ mr: 2 }}>
                <ArrowBackIcon />
            </IconButton>
    
            {id ? 'Editar Actividad' : 'Formulario para registro de las asistencias a actividades o reuniones que realizan los diferentes contratistas o nombrados de la alcaldía municipal de de Quibdó'}
                                    ...
                                    <>
                                        <TableContainer component={Paper}>
                                            <Table size="small">
                                                ...
                                                <TableBody>
                                                    {Array.isArray(currentAsistentes) && currentAsistentes.length > 0 ? (
                                                        currentAsistentes.map((asistente, index) => {
                                                            // Si es un beneficiario, buscamos su información
                                                            if (asistente.tipo === 'beneficiario' || !asistente.tipo) {
                                                                const beneficiario = getBeneficiarioInfo(asistente.beneficiario_id || asistente._id);
                                                                return (
                                                                    <TableRow key={`benef-${asistente.beneficiario_id || asistente._id || index}`}>
                                                                        <TableCell>
                                                                            {beneficiario ? (
                                                                                <>
                                                                        ...
                                                                        <TableCell align="left">
                                                                            {beneficiario?.tipo_documento ? `${beneficiario.tipo_documento}: ${beneficiario.numero_documento || 'N/A'}` : 'N/A'}
                                                                        </TableCell>
                                                            ...
                                                            // Si es un funcionario, mostramos su información directamente
                                                            else if (asistente.tipo === 'funcionario') {
                                                                return (
                                                                    <TableRow key={`func-${asistente.funcionario_id || asistente._id || index}`}>
                                                                        <TableCell>
                                                                            {asistente.nombre || 'Funcionario sin nombre'}
                                                                        </TableCell>
                                                                        <TableCell align="left">
                                                                            C.C: {asistente.cedula || 'N/A'}
                                                                        </TableCell>
                                                                        <TableCell align="center">
                                                                            {asistente.asistio ? (
                                                                                <Chip 
                                                            ...
                                                            // Por defecto, en caso de que no coincida con ningún tipo conocido
                                                            return (
                                                                <TableRow key={`unknown-${index}`}>
                        ...
                        <CardContent>
                            ...
                            <List>
                                ...
                                <ListItem>
                                    <ListItemText 
                                        primary="Habitantes" 
                                        secondary={asistentes.filter(a => !a.tipo || a.tipo === 'beneficiario').length}
                                ...
                                <ListItem>
                                    <ListItemText 
                                        ...
                                        secondary={asistentes.filter(a => a.tipo === 'funcionario').length}
                                        secondaryTypographyProps={{ variant: 'body1' }}
                                    />
                                </ListItem>
                                <ListItem>
