 ...
 * @param {number} options.pagina - Número de página (por defecto: 1)
 * @param {number} options.porPagina - Cantidad de registros por página (por defecto: 10)
 * @param {string} options.busqueda - Término de búsqueda
 * @param {string} options.campoOrden - Campo por el que ordenar
 * @param {string} options.direccionOrden - Dirección de ordenación (asc/desc)
 * @returns {Promise<Object>} Objeto con datos de paginación y lista de actividades
 */
export const getActividades = async ({
    ...
    try {
        const params = {
            ...
            orden_por: campoOrden,
            orden_direccion: direccionOrden,
        };
        ...
        const response = await axiosInstance.get(ACTIVIDADES_API, { params });
        ...
        if (response.data && response.data.data) {
            return {
                ...
                pagina: response.data.pagina_actual || pagina,
                totalPaginas: response.data.total_paginas || Math.ceil((response.data.total || response.data.data.length) / porPagina)
            };
        }
        
        // Si la respuesta no tiene la estructura esperada, devolver los datos como están
        return {
        ...
        throw new Error(error.response?.data?.message || 'No se pudieron cargar las actividades');
    }
};
