...
from flask import request, current_app, jsonify, send_file, g, json
...
def crear_actividad():
    try:
        ...
        try:
            ...
            # Asegurarse de que tipo_actividad tenga un valor válido
            if 'tipo_actividad' not in datos or datos['tipo_actividad'] not in ['reunion', 'programa']:
                datos['tipo_actividad'] = 'programa'
...
def obtener_actividades():
    try:
        ...
        resultado = actividad_model.obtener_actividades_paginadas(
            pagina=pagina,
        ...
        logger.error(f"Error al obtener actividades: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
...
def guardar_asistentes(actividad_id):
    ...
    try:
        ...
        try:
            datos = request.get_json()
            if not datos or 'asistentes' not in datos:
                return jsonify({
                    'success': False,
                    'message': 'Se requiere la lista de asistentes'
                }), 400
            ...
            logger.error(f"Error al procesar los datos de la solicitud: {str(e)}")
            return jsonify({
                'success': False,
        ...
        # Guardar los asistentes
        actividad_model = ActividadModel()
        try:
            num_asistentes = actividad_model.guardar_asistentes(
            ...
            return jsonify({
                'success': True,
                'message': f'Se guardaron {num_asistentes} asistentes correctamente',
                'count': num_asistentes
            })
            ...
            logger.error(f"Error inesperado al guardar asistentes: {str(e)}")
            return jsonify({
                'success': False,
        ...
        logger.error(f"Error en guardar_asistentes: {str(e)}")
        return jsonify({
            'success': False,
